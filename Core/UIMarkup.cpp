...
bool CMarkup::_Parse(LPTSTR& pstrText, ULONG iParent)
{
    ...
    for( ; ; ) 
    {
        ...
        if( *pstrText != _T('<') )	//一定要是开始标签 <>
			return _Failed(_T("Expected start tag"), pstrText);
        if( pstrText[1] == _T('/') ) //说明是结束标签 </>
			return true;
        *pstrText++ = _T('\0');
        _SkipWhitespace(pstrText);
        // Skip comment or processing directive 忽略注释
        ...
        XMLELEMENT* pEl = _ReserveElement();	//返回一个XMLELEMENT的空间
        ULONG iPos = pEl - m_pElements;	//pEl在整个数组的位置
